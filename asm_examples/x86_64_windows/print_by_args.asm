...
; code += b'\x48\x8D\x05\x2B\x01\x00\x00'                                      # LEA    RAX,[RIP+0x12B]             # 0x138  ; manually modified offset in shellcode
; code += b'\x48\x8D\x1D\xFC\x00\x00\x00'                                      # LEA    RBX,[RIP+0xFC]              # 0x110  ; manually modified offset in shellcode
; code += b'\x48\x89\x43\x08'                                                  # MOV    QWORD PTR [RBX+0x8],RAX
; code += b'\x48\x8D\x05\xC1\x00\x00\x00'                                      # LEA    RAX,[RIP+0xC1]              # 0xE0   ; manually modified offset in shellcode
; code += b'\x48\x89\x58\x10'                                                  # MOV    QWORD PTR [RAX+0x10],RBX
; code += b'\x48\x83\xEC\x30'                                                  # SUB    RSP,0x30
; code += b'\x4C\x8D\x15\x02\x01\x00\x00'                                      # LEA    R10,[RIP+0x102]             # 0x130  ; manually modified offset in shellcode
; code += b'\x48\xC7\xC2\x16\x01\x12\x00'                                      # MOV    RDX,0x120116
; code += b'\x4C\x8D\x05\xA4\x00\x00\x00'                                      # LEA    R8,[RIP+0xA4]               # 0xE0   ; manually modified offset in shellcode
; code += b'\x4C\x8D\x0D\xDD\x00\x00\x00'                                      # LEA    R9,[RIP+0xDD]               # 0x120  ; manually modified offset in shellcode
...
; code += b'\x4C\x8B\x15\xBF\x00\x00\x00'                                      # MOV    R10,QWORD PTR [RIP+0xBF]    # 0x130  ; manually modified offset in shellcode
...
; code += b'\x48\x8D\x05\x93\x00\x00\x00'                                      # LEA    RAX,[RIP+0x93]              # 0x120  ; manually modified offset in shellcode
...
; code += b'\x4C\x8B\x15\x66\x00\x00\x00'                                      # MOV    R10,QWORD PTR [RIP+0x66]    # 0x130  ; manually modified offset in shellcode
